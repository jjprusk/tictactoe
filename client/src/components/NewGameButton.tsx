...
export default function NewGameButton(): JSX.Element {
  ...
  useEffect(() => {
    ...
    // As a fallback in tests/environments, also react to storage events
    window.addEventListener('storage', updateReady as any);
    return () => {
      window.removeEventListener('ttt:session-change' as any, updateReady as any);
      window.removeEventListener('storage', updateReady as any);
    };
  }, []);
...
export function NewGameButtonSecondary(): JSX.Element {
  const dispatch = useDispatch();
  const { gameId } = useSelector(selectGame);
  const [ready, setReady] = useState<boolean>(() => {
    try {
