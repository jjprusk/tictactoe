...
describe('makeMove orchestrator', () => {
  it('uses ai0 strategy to pick a legal move', async () => {
    const board = [null, 'X', null, null, 'O', null, null, null, null] as any;
    const pos = await makeMove(board, 'X', 'ai0' as any);
  ...
  it('returns -1 when no legal moves', async () => {
    ...
    const pos = await makeMove(full, 'O', 'ai0' as any);
    expect(pos).toBe(-1);
  });

  it('records latency metric even when ai0 (random) throws', async () => {
